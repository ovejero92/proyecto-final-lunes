const productos = [
    {nombre:'Remera',precio:7526,stock:0,img:'remera-negra.jpeg',categoria:'Ropa'},
...
function mostrarProductos(){
    let container = document.getElementById('CP')
    container.innerHTML = '';
...
function agregarProductoAlCarrito(nombre){
    ...
    if(itemEnCarrito){
        if (itemEnCarrito.cantidad < producto.stock) {
            ...
            alert('no hay suficioente stock')
            itemEnCarrito.cantidad
        }
    } else {
...
function obtenerTotal(){
    return carrito.reduce((total,item) => total + item.producto.precio * item.cantidad,0).toFixed(2)
}
